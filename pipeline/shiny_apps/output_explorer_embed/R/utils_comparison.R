...
build_pathway_summary_gtable <- function(long,
  ...
  # Both pathway methods behave as non-directional here (MAGMA uses a
  # one-sided competitive gene-set test; TWAS-GSEA non-dir emits a
  # right-tail P). Match the Drug summary's `pos_data` style verbatim:
  # shape 16, colour = Z, white -> green, positive-only limits.
